...
import {
  generateIdeasWithLocalCli,
  getBuildJob,
  getLatestBuildJob,
  getLocalProviderState,
  openBuildProject,
  startLocalBuild,
} from "./desktop/local-factory.mjs";
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (requestUrl.pathname === "/api/open-project") {
      await handleOpenProjectRequest(req, res);
      return;
    }
    ...
    if (requestUrl.pathname === "/api/health") {
      ...
      sendJson(res, 200, {
        ...
        ...localState,
        latestBuildJob: getLatestBuildJob(),
      });
      return;
    }

    if (requestUrl.pathname === "/api/build-status") {
      await handleBuildStatusRequest(requestUrl, res);
      return;
    }
...
async function handleBuildRequest(req, requestUrl, res) {
  ...
  try {
    ...
    sendJson(res, 400, { error: "Build request body is invalid." });
    return;
  }

  const provider = normalizeProvider(body.provider);
  const idea = normalizeBuildIdea(body.idea);
  const categoryLabel = cleanSentence(body.categoryLabel) || "General";
  const intensityLabel = cleanSentence(body.intensityLabel) || "Normal";

  if (!idea.title || !idea.idea) {
    sendJson(res, 400, { error: "Build request is missing idea details." });
    return;
  }
  ...
  try {
    ...
    sendJson(res, 400, {
      error: error instanceof Error ? error.message : "Build could not be started.",
    });
  }
}

async function handleBuildStatusRequest(requestUrl, res) {
  const jobId = normalizeBuildJobId(requestUrl.searchParams.get("id"));
  const job = jobId ? getBuildJob(jobId) : getLatestBuildJob();
...
async function handleOpenProjectRequest(req, res) {
  if (req.method !== "POST") {
    sendJson(res, 405, { error: "Use POST to open a project folder." });
    return;
  }
  ...
  try {
    ...
    sendJson(res, 400, { error: "Open-project request body is invalid." });
    return;
  }

  const jobId = normalizeBuildJobId(body.jobId);
  if (!jobId) {
    sendJson(res, 400, { error: "Open-project request is missing a valid job id." });
    return;
  }

  const opened = await openBuildProject(jobId);
  if (!opened) {
    sendJson(res, 404, { error: "Build job not found." });
    return;
  }
...
async function generateIdeas({ category, provider, intensityValue, intensityBand, signals }) {
  const buildBackupIdeas = () =>
  ...
  const localIdeas = await generateIdeasWithLocalCli({
    rootDir,
    provider,
  ...
  if (localIdeas.ideas?.length) {
    return {
      ideas: ensureIdeaCount(localIdeas.ideas, buildBackupIdeas),
      provider: localIdeas.provider,
      providerLabel:
        localIdeas.provider === "claude"
          ? "Claude Code local"
          : localIdeas.provider === "codex"
            ? "Codex local"
            : "Local CLI",
      warning: localIdeas.warning || null,
    };
  }
  ...
  if (groqIdeas?.length) {
    return {
      ideas: ensureIdeaCount(groqIdeas, buildBackupIdeas),
      provider: "groq",
  ...
  return {
    ideas: buildBackupIdeas(),
    provider: "fallback",
    providerLabel: "Backup ideas",
    warning: localIdeas.warning || null,
  };
}
...
function ensureIdeaCount(ideas, buildBackupIdeas) {
  const primaryIdeas = Array.isArray(ideas) ? ideas.filter(Boolean).slice(0, IDEA_COUNT) : [];

  if (primaryIdeas.length >= IDEA_COUNT) {
    return primaryIdeas;
  }

  const seenKeys = new Set(
    primaryIdeas
      .map((idea) => `${cleanSentence(idea.title).toLowerCase()}::${cleanSentence(idea.idea).toLowerCase()}`)
      .filter(Boolean),
  );

  const fillerIdeas = buildBackupIdeas().filter((idea) => {
    const key = `${cleanSentence(idea.title).toLowerCase()}::${cleanSentence(idea.idea).toLowerCase()}`;
  ...
  return [...primaryIdeas, ...fillerIdeas].slice(0, IDEA_COUNT);
}
...
function deriveThemes(category, signals) {
  ...
  const themes = [...new Set([...matchedThemes, ...pool])].slice(0, IDEA_COUNT);
...
function normalizeBuildJobId(value) {
  if (typeof value !== "string") {
    return "";
  }
...
function normalizeBuildIdea(value) {
  return {
    title: truncate(cleanSentence(value?.title), 60),
    idea: truncate(cleanSentence(value?.idea), 220),
    why: truncate(limitSentences(cleanSentence(value?.why), 2), 220),
    starterPrompt: truncate(cleanSentence(value?.starterPrompt), 260),
    sourceTitles: Array.isArray(value?.sourceTitles)
      ? value.sourceTitles.map((item) => truncate(cleanSentence(item), 120)).filter(Boolean).slice(0, 3)
      : [],
  };
}
